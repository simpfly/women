...
import { subscribeBusuanzi } from './utils/busuanzi';
...
const App: React.FC = () => {
  ...
  // 监听全网真实独立访客打卡 (Busuanzi UV)
  useEffect(() => {
    const unsubscribe = subscribeBusuanzi((stats) => {
      if (stats.siteUv && stats.siteUv > 0) {
        setVisitorCount(stats.siteUv);
      }
    });
    return () => unsubscribe();
  }, []);
